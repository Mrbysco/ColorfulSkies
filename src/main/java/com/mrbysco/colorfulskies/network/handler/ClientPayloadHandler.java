...
public class ClientPayloadHandler {
	...
	public void handleCloudData(final CloudColorPayload data, final IPayloadContext context) {
		context.enqueueWork(() -> {
			...
			// Handle exception
			context.disconnect(Component.translatable("colorfulskies.networking.cloud_color.failed", e.getMessage()));
			return null;
		});
	}

	public void handleMoonData(final MoonColorPayload data, final IPayloadContext context) {
		context.enqueueWork(() -> {
			int color = data.color();
			if (color == -1) {
			...
			// Handle exception
			context.disconnect(Component.translatable("colorfulskies.networking.moon_color.failed", e.getMessage()));
			return null;
		});
	}

	public void handleSunData(final SunColorPayload data, final IPayloadContext context) {
		context.enqueueWork(() -> {
			int color = data.color();
			if (color == -1) {
			...
			// Handle exception
			context.disconnect(Component.translatable("colorfulskies.networking.sun_color.failed", e.getMessage()));
			return null;
		});
	}

	public void handleSunriseData(final SunriseColorPayload data, final IPayloadContext context) {
		context.enqueueWork(() -> {
			int color = data.color();
			if (color == -1) {
			...
			// Handle exception
			context.disconnect(Component.translatable("colorfulskies.networking.sunrise_color.failed", e.getMessage()));
			return null;
		});
	}

	public void handleSkyData(final SkyColorPayload data, final IPayloadContext context) {
		context.enqueueWork(() -> {
			int color = data.color();
			if (color == -1) {
			...
			// Handle exception
			context.disconnect(Component.translatable("colorfulskies.networking.sky_color.failed", e.getMessage()));
			return null;
		});
	}

	public void handleDisableData(final DisableSunrisePayload data, final IPayloadContext context) {
		context.enqueueWork(() -> {
			com.mrbysco.colorfulskies.client.ClientHandler.sunriseDisabled = data.disabled();
		}).exceptionally(e -> {
			// Handle exception
			context.disconnect(Component.translatable("colorfulskies.networking.disable_sunrise.failed", e.getMessage()));
			return null;
		});
	}
}
