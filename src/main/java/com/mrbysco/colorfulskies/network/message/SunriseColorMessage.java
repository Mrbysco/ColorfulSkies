...
public class SunriseColorMessage {
	...
	public SunriseColorMessage(int color) {
		this.color = color;
	}

	public static SunriseColorMessage decode(final FriendlyByteBuf buffer) {
		return new SunriseColorMessage(buffer.readInt());
	}
			...
			return new SafeRunnable() {
				...
				public void run() {
					if (color == -1) {
						...
						int g = (color >> 8) & 0xFF;
						int b = (color >> 0) & 0xFF;
						ClientHandler.setSunriseColor(new Color((float) r / 255.0F, (float) g / 255.0F, (float) b / 255.0F));
					}
				}
			};
		}
